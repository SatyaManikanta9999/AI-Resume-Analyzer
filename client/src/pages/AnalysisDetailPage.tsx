...
export default function AnalysisDetailPage() {
  ...
  return (
    <div className="p-4 sm:p-8 max-w-6xl mx-auto">
      ...
      {/* Header */}
      <div className="flex items-start justify-between mb-6 sm:mb-8 animate-slide-up">
        <div className="min-w-0">
          ...
          <h2 className="font-display font-bold text-2xl sm:text-3xl text-white truncate">{analysis.jobTitle}</h2>
      ...
      {/* ── Score cards ─────────────────────────────────────
          ...
          Desktop: 1×4 row   (sm:grid-cols-4)
      ────────────────────────────────────────────────────── */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-8 animate-slide-up">
        {[
          ...
          <div key={label} className="glass-card p-3 sm:p-5 flex flex-col items-center gap-2 sm:gap-3">
            {/* Smaller ring on mobile (70px), normal on desktop (90px) */}
            ...
            <p className="text-slate-400 text-xs sm:text-sm text-center font-medium leading-tight">{label}</p>
          </div>
        ))}
      </div>

      {/* Summary */}
      <div className="glass-card p-5 sm:p-6 mb-6 border-brand-500/20 bg-brand-500/5 animate-slide-up">
      ...
      {/* ── Radar + Keyword Density ──────────────────────────
          Mobile:  stacked (grid-cols-1)
          Desktop: side-by-side (sm:grid-cols-2)
      ────────────────────────────────────────────────────── */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-6 animate-slide-up">
      ...
      <div className="space-y-4 animate-slide-up">
        <SectionCard title="Strengths"             items={analysis.sections.strengths}         icon={CheckCircle2} color="text-emerald-400" iconBg="bg-emerald-500/10" />
        <SectionCard title="Weaknesses"            items={analysis.sections.weaknesses}        icon={XCircle}      color="text-rose-400"    iconBg="bg-rose-500/10"    />
        <SectionCard title="Suggestions to Improve" items={analysis.sections.suggestions}     icon={Lightbulb}    color="text-amber-400"   iconBg="bg-amber-500/10"   />
        {analysis.atsCompatibility.issues.length > 0 && (
          <SectionCard title="ATS Compatibility Issues" items={analysis.atsCompatibility.issues} icon={Shield}    color="text-brand-400"   iconBg="bg-brand-500/10"   />
        )}
      </div>
